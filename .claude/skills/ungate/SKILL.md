...
2. `Docs/tickets/GATE_SPEC.md` — the format you're reading and (at the end) rewriting. Its "Claiming a number" section is the canonical claim-**by-push** convention every gate-stub filer — `ticket-writer` step 3, the executor's `EXECUTOR_ROUTINE.md` Step 3, and this skill's own ticket-drafting path below (step 4) — follows to keep two concurrent sessions from computing the same `G-###`/`T-###`.
3. `Docs/tickets/TICKET_SPEC.md` — the format for whatever ticket(s) this resolution produces.

## Procedure

0. Create this session's own worktree for the gate-resolution branch — never cut it in the shared primary directory (`Docs/IMPLEMENTATION_NOTES.md` § T-069/T-070): `git fetch origin develop && git worktree add tmp/worktrees/<gate-slug>/ -b gates/<gate-slug> origin/develop` (e.g. `gates/g-004-ocr-approach`), then `cd` into it. Docs-only (plus a mockup, for a 🎨 gate) — PR'd into `develop` like ticket-creation branches, using a distinct `gates/*` prefix for the same reason `tickets/*` is distinct from `feat/*`: a `git branch -a` scan should show gate-resolution PRs apart from both ticket-planning and ticket-implementation PRs at a glance. Only where the branch is cut has changed — the `gates/*` prefix itself is unchanged.
...
3. Work the `Open question:` to an actual decision, together with Alex. This is the reason the gate exists — do not invent an answer and do not proceed past this step without one.
   - **🎨 gate**: produce (or confirm an already-drafted) mockup at `Docs/mockups/<view>/` (index.html + screenshot.png + NOTES.md), per the usual mockup convention. The mockup *is* the resolution — once it exists, the gate is cleared the same way a ticket referencing a mockup is never visually gated (`AGENTS.md`).
...
4. Now that the decision exists, resolve every consumer named in `Blocks:`:
   ...
   - **No ticket exists yet for a named milestone task**: draft it now, following `.claude/skills/ticket-writer/SKILL.md`'s procedure (including its claim-then-draft numbering step — commit **and push** a placeholder `T-###` file the moment the number is chosen, before drafting the rest — a commit alone doesn't count, since this session works in its own worktree per `T-069`) and `TICKET_SPEC.md`'s format exactly (same sizing rule, same field discipline), landing it in `queue/` or `backlog/` as that procedure dictates. Replace that task's `(Gated on: G-###)` tag with `(T-###)` on the milestone doc — same tagging obligation `ticket-writer` itself has when it drafts a ticket.
...
6. If the decision is durable and non-obvious enough to matter beyond this one gate, add a pointer to `Docs/IMPLEMENTATION_NOTES.md` (one line, per `AGENTS.md`'s "WHY only, once" comment rule — the full rationale lives on the gate-stub's resolution, not duplicated in both places).
