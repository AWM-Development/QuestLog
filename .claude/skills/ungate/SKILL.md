...
4. Now that the decision exists, resolve every consumer named in `Blocks:`:
   ...
   - **No ticket exists yet for a named milestone task**: draft it now, following `.claude/skills/ticket-writer/SKILL.md`'s procedure (including its claim-then-draft numbering step — commit a placeholder `T-###` file the moment the number is chosen, before drafting the rest) and `TICKET_SPEC.md`'s format exactly (same sizing rule, same field discipline), landing it in `queue/` or `backlog/` as that procedure dictates. Replace that task's `(Gated on: G-###)` tag with `(T-###)` on the milestone doc — same tagging obligation `ticket-writer` itself has when it drafts a ticket.
