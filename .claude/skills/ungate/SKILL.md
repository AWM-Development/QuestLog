...
1. List `Docs/tickets/gated/*.md`, sort by `G-###` numerically, pick the lowest. **Always the earliest — never cherry-pick a later one**, same convention as the nightly executor always picking the earliest queued `T-###` (`EXECUTOR_ROUTINE.md` Step 1): predictable order, no incentive to dodge the oldest open question.
...
4. Now that the decision exists, resolve every consumer named in `Blocks:`:
   ...
   - **No ticket exists yet for a named milestone task**: draft it now, following `.claude/skills/ticket-writer/SKILL.md`'s procedure and `TICKET_SPEC.md`'s format exactly (same sizing rule, same field discipline), landing it in `queue/` or `backlog/` as that procedure dictates.
5. **Sync safety net** — independently grep all of `backlog/`, `queue/`, and `in-progress/` for `Gated on: G-###` (this gate's id), including any match `Blocks:` didn't already name. Clear every one the same way as step 4. Do not assume `Blocks:` was exhaustive — `GATE_SPEC.md`'s "Keeping tickets and gates in sync" treats this sweep as the actual guarantee, not the `Blocks:` list.
...
9. Report back: the gate id and its resolution, every ticket created or promoted (and where each landed), any `Gated on:` reference the step-5 sweep caught that `Blocks:` had missed, and whether any other open gate-stub remains for a future `/ungate` run.
