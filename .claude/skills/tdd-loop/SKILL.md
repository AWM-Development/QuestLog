...
- **Real-API `.e2e.test.ts` tests are the occasional exception, not a per-ticket default.** They exist to answer a different question than the mocked suite does — "does the real model/account integration still work" — which is a fact about the vendor and the account, not about this PR's code, so it doesn't need re-proving on every commit. They run on their own schedule (`.github/workflows/e2e-nightly.yml`, `pnpm test:e2e`), never as part of the default `pnpm test` / PR gate (see `Docs/IMPLEMENTATION_NOTES.md` for why — it was tried, and a shared per-vendor rate limit made it an unreliable merge gate). Only add a new one of these if you're genuinely proving a new real-API integration path for the first time, matching the existing pattern (`describe.skipIf(!process.env.VOYAGE_API_KEY)`, permanent fixture reuse, excluded from the default vitest config) — don't add one as a matter of course for every ticket that happens to touch an external API.
