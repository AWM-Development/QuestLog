...
Each DB-touching package has its own physical test database on `:5433` (docker-compose) — `questlog_test_core` (`packages/core`), `questlog_test_server` (`apps/server`), `questlog_test_mcp` (`packages/mcp`/`apps/mcp-stdio`); canonical list in `scripts/test-db-names.sh` (T-071). A worktree gets its own dedicated Postgres container on its own port, derived from `process.cwd()` rather than an env var a session has to remember to export (`packages/core/src/db/test-db-url.ts`'s `resolveWorktreePort()`) — a command run directly, with no setup script sourced first, still resolves the right port. `global-setup.ts` truncates tables between test files but does **not** run migrations — run `db:migrate` against each one manually after pulling a new migration, or tests fail with missing-column errors.
