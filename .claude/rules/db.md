...
Each DB-touching package has its own physical test database on the one shared local Postgres instance, `:5433` (docker-compose, `name: questlog` — every worktree's `docker compose up -d` converges on this same container, T-154) — `questlog_test_core` (`packages/core`), `questlog_test_server` (`apps/server`), `questlog_test_mcp` (`packages/mcp`/`apps/mcp-stdio`); canonical list in `scripts/test-db-names.sh` (T-071). Inside a worktree, every one of these names is suffixed with that worktree's own name (`questlog_test_core__t_109`) — derived from `process.cwd()`, not an env var, so it resolves correctly even for a command run directly with no setup script sourced first (`packages/core/src/db/test-db-url.ts`'s `resolveWorktreeDbSuffix()`, T-154) — which is what actually isolates concurrent worktrees now, not a per-worktree port. `global-setup.ts` truncates tables between test files but does **not** run migrations — run `db:migrate` against each one manually after pulling a new migration, or tests fail with missing-column errors.
