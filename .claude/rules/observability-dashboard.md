...
rendering JSX) and `utils/` (pure types/functions the components consume,
no JSX). Named `utils/`, not `lib/`, deliberately: `src/lib/` already
exists one level up as this app's cross-feature infra folder (`trpc.ts`,
mirroring `apps/web/src/lib/`) — a same-named `lib/` nested inside a
feature would mean two different things sharing one name at different
depths. There's no fixed file-count threshold for when to split; the
signal is a flat listing no longer being scannable at a glance, the same
reasoning `apps/web`'s own subfolder split exists for.
