...
`Docs/PRD.md` §8 names this database as holding "irreplaceable campaign lore" as a technical risk mitigation concern (RAG quality risk row) without specifying a backup policy. Both Fly.io MPG and Railway's managed-Postgres plugin advertise automated backups as part of their base offering (§2.1) — but retention window, point-in-time-recovery availability, and whether backups are tested (a restore drill, not just "backups exist") are unresolved and depend on §2.1's choice. Not decided here.
