...
The pipeline replaced the old 🎨 (visual spec) gate with mockups: a ticket whose `Mockup:` field names a path is *not* visually gated — the mockup, generated by Alex in Claude Design during planning, is the answer (`Docs/mockups/README.md`). 🧠 (strategy discussion required) still stops the executor cold on that specific item: it skips it, files a gate-stub in `Docs/tickets/gated/` (`Docs/tickets/GATE_SPEC.md`) instead of leaving the gap to be rediscovered only in the morning report, and continues with whatever in the ticket doesn't depend on it (`CLAUDE.md` hard rules, `Docs/tickets/EXECUTOR_ROUTINE.md` Step 3). Resolving that gate-stub is an interactive `/ungate` session with Alex (`.claude/skills/ungate/SKILL.md`) — never something the executor decides on its own later. The cost of pausing for clarity is a few minutes; the cost of building the wrong thing is an entire session.
