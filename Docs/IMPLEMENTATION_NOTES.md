...
### T-043 — the bash/YAML name list itself deduped into `scripts/test-db-names.sh`, except `session-start.sh`
The three-way hand-copied `questlog`/`questlog_test`/`questlog_test_mcp` list described above is now `scripts/test-db-names.sh` (`TEST_DB_NAME_DEV`/`TEST_DB_NAME_UNIT`/`TEST_DB_NAME_MCP` plus a `TEST_DB_NAMES` array), sourced by `ci.yml` and `e2e-release-check.yml`. GitHub Actions' `env:` block can't reference a shell variable sourced inside `run:`, so both workflows' `DATABASE_URL` for the mcp test DB is now built inside the `run:` script (`export DATABASE_URL="postgresql://questlog:questlog@localhost:5433/${TEST_DB_NAME_MCP}"`) rather than hardcoded in `env:`, which is what let the literal disappear from those two files entirely rather than just moving to a different hardcoded spot.

`.claude/hooks/session-start.sh` still hand-lists `questlog questlog_test questlog_test_mcp` (its own copy, not sourcing the canonical file) — the same sandbox tool restriction noted just above (this file can't be edited in this environment; it's the registered `SessionStart` hook command itself, per `.claude/settings.json`) blocked this ticket's attempt too. If a fourth test database is ever added, `session-start.sh`'s copy is the one place a manual, out-of-band edit is still required.
