...
### `tmp/.active-ticket` resolves per-worktree for free; `tmp/.session-context.<session_id>.json` needed an explicit fix
`resolveActiveTicketId`/`captureUsage` (`packages/core/src/observability/capture-usage.ts`) resolve `tmp/.active-ticket` against `process.env.CLAUDE_PROJECT_DIR ?? process.cwd()`. `EXECUTOR_ROUTINE.md` Step 6/7 invoke `capture-usage` as a plain shell command (not a hook), and empirically, in that invocation context `CLAUDE_PROJECT_DIR` is unset (confirmed by inspecting the environment available to a Bash-tool-family command directly, during this ticket's own execution) — so it falls back to `process.cwd()`, which is the worktree once Step 2 has `cd`'d into it. Each worktree is a genuinely separate directory tree (verified empirically: `git worktree add` for two ticket ids, then writing a file under one worktree's `tmp/` and confirming it's absent from the other's), so this marker needed no code change — the fix is entirely in `EXECUTOR_ROUTINE.md`'s Step 2 now running inside the worktree instead of the primary directory.
