...
### `appendContent` is not campaign-scoped, unlike `getByIdForCampaign`
`sourceService.appendContent(db, sourceId, content)` (`packages/core/src/services/source.service.ts`) looks up the source via `getById`, not `getByIdForCampaign` — even though the latter exists specifically for untrusted external input (e.g. an MCP tool) per its own docstring, and `ingest_text`'s handler already has `campaignId` in scope on the append branch. This matches the ticket's literally-specified signature (`appendContent(db, sourceId, content)`, no `campaignId` param) and doesn't break any exit condition, but it means a `sourceId` for a source in a different campaign would silently succeed today. Flagged by T-065's review pass; worth tightening in a follow-up ticket if cross-campaign `sourceId` guessing becomes a real concern (single-user v1 makes this low-severity for now).

### New `ValidationError` from `appendContent` isn't mapped in `withToolErrors`
`packages/mcp/src/tools/errors.ts`'s `withToolErrors` only special-cases `NotFoundError` — the `ValidationError` `appendContent` throws when a source isn't `pending` falls through to the MCP SDK's own generic catch, which returns `isError: true` with a bare message string rather than `.claude/rules/mcp.md`'s `{ error: { code, message } }` shape. The ticket's exit condition only requires the call to throw (satisfied), so this ships as-is; a future ticket touching `errors.ts` should add a `ValidationError` branch (`code: "VALIDATION_ERROR"` or similar) for consistency across tools.
