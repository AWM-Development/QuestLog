...
`tmp/.session-context.json` (written by `.claude/hooks/session-start.sh`) is different: that hook fires at session start, *before* any worktree exists (the ticket id a worktree would be named for isn't even known yet — Step 1 hasn't run), using `CLAUDE_PROJECT_DIR` as set by the harness for the hook's own invocation. That value is fixed to the sandbox/primary root for the life of the session — it does not follow a later `cd`/`git worktree add` the agent performs — so the file was landing in the *primary* directory's `tmp/` regardless of which worktree the session later works in. A flat filename there is a real collision: two concurrent sessions both starting up on one shared physical checkout each overwrite the same file, silently misattributing whichever wrote second's transcript path to Step 7's usage capture (this predates T-069 — it was already true, just newly documented here). Fixed by keying the filename on `session_id` (already present in the hook's own stdin payload): `tmp/.session-context.<session_id>.json`. `EXECUTOR_ROUTINE.md` Step 6/7 read it back via `CLAUDE_CODE_SESSION_ID` (already an established env var in this codebase — `capture-usage.ts`'s own `resolveHookPayloadFromEnv` fallback, T-035, already depends on its presence) combined with `git rev-parse --path-format=absolute --git-common-dir` to locate the primary directory from inside any worktree (the common `.git` dir always lives at the primary root, regardless of which worktree the query runs from).
