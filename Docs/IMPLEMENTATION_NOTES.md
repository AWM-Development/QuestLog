...
`.env` is gitignored (`.gitignore:2`) and never committed, so a fresh clone/sandbox (exactly what the nightly executor runs in) starts with no `DATABASE_URL` at all. Three independent points confirm nothing fills that gap with a real Neon/Fly secret:
