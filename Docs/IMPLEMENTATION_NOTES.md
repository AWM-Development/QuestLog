...
1. **The premise was avoidable, not just wrong.** Routines have a per-repository setting — "Allow unrestricted git push — Let the agent push to any branch on this repo, including the default branch" — that explicitly sanctions pushing non-`claude/*` branch names. With it enabled (QuestLog's nightly routine, 2026-07-16), `EXECUTOR_ROUTINE.md` Step 2 simply creates and pushes the ticket's `Branch:` name directly; the PR is born on the convention name and no post-hoc rename is needed. (T-004's empirical finding below had already shown new-branch pushes weren't technically blocked — the workflow existed only so the routine wouldn't disobey the harness's default policy instruction, which the routine's own config now overrides.)
...
- The mechanical guard keeping agents off `main`/`develop` is no longer the Anthropic git proxy (unrestricted push disables it). It is now (a) the CRITICAL BRANCH RULES in `EXECUTOR_ROUTINE.md`, and (b) GitHub branch-protection rulesets on `main` and `develop` (require PR, block force pushes, restrict deletions) that Alex maintains in repo settings — the layer that actually can't be prompt-injected around.
- The "babysitting a renamed PR" caveat below is dead: a PR's head branch now matches the ticket's `Branch:` field from birth, so follow-up sessions can push to it directly. Looking up a PR's current head ref via `gh` before pushing remains cheap insurance.
