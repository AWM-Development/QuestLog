...
Drizzle doesn't have native pgvector support. `apps/server/src/db/schema/custom-types.ts` defines a `vector(name, dimensions)` custom column type that serializes `number[]` ↔ Postgres `vector(N)` format. The `fromDriver` method handles null/empty edge cases defensively.
