...
- Invoke usage-capture directly, synchronously, instead of waiting for the `Stop` hook to fire (which wouldn't happen until after this turn ends, i.e. after the PR is already open): `cat .claude/.session-context.json | pnpm --filter @questlog/server run capture-usage`. This reuses the exact CLI entrypoint and stdin payload shape the `Stop` hook already feeds it — `.claude/.session-context.json` was stashed with that identical shape by `session-start.sh` at session start. Include the resulting `Docs/tickets/cost-reports/T-###.usage.json` in the same commit as the rest of this wrap-up.
- Commit all of the above.
- Delete `.claude/.active-ticket` — this session's ticket work is done, and the marker must not linger to misattribute a later, unrelated session's usage.
