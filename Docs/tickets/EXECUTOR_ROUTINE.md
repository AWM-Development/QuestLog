...
This entire step is read-only against `origin/develop` and `origin`'s refs — nothing here touches a working tree, so it's safe to run from the shared primary directory before any worktree exists (the ticket id a worktree would be named for isn't known until this step finishes).
...
- If nothing matches, `git ls-remote --heads origin` for a branch containing the ticket's nominal `Branch:` field — catches pre-PR work (blocked, interrupted, or actively claimed) that still used the nominal name.
