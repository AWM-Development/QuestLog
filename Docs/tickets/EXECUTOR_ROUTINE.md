...
- Invoke usage-capture directly, synchronously, same as Step 7: `cat tmp/.session-context.json | pnpm --filter @questlog/server run capture-usage`. A blocked run still burns real tokens and should have a cost record; include the resulting `Docs/tickets/cost-reports/T-###.usage.json` in the same commit as the blocked report.
...
- Invoke usage-capture directly, synchronously, instead of waiting for the `Stop` hook to fire (which wouldn't happen until after this turn ends, i.e. after the PR is already open): `cat tmp/.session-context.json | pnpm --filter @questlog/server run capture-usage`. This reuses the exact CLI entrypoint and stdin payload shape the `Stop` hook already feeds it — `tmp/.session-context.json` was stashed with that identical shape by `session-start.sh` at session start. Include the resulting `Docs/tickets/cost-reports/T-###.usage.json` in the same commit as the rest of this wrap-up.
