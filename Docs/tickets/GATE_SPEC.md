...
`/ungate` (`.claude/skills/ungate/SKILL.md`) is the only mechanism that resolves a gate-stub. Like the nightly executor pulling the earliest queued `T-###`, `/ungate` always pulls the earliest open `G-###` in numeric order — same "oldest first, no cherry-picking" convention, for the same reason: predictable order that doesn't depend on whoever happens to run the command noticing a different one first.
