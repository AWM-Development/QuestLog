...
Context files (load ONLY these):
  ...
  - packages/core/src/observability/pricing.ts (existing token-cost accounting — check whether the new call path should feed the same usage-tracking mechanism `conversation.service.ts` already uses, so a new LLM call site isn't invisible to cost observability)
