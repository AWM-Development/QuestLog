...
Context files (load ONLY these):
  ...
  - scripts/test-db-names.sh (canonical name list — gains one new name per package)
  - .claude/hooks/session-start.sh (already loops `TEST_DB_NAMES` — should need zero new lines)
  - .github/workflows/ci.yml, .github/workflows/e2e-release-check.yml (both currently provision `questlog_test_mcp` via one hardcoded step, not a loop — this is what must become generic)
...
4. **Rewrite `ci.yml`'s and `e2e-release-check.yml`'s test-database provisioning into a loop over `scripts/test-db-names.sh`'s array**, replacing the current single hardcoded "create and migrate `questlog_test_mcp`" step in each file. This is the load-bearing part of the ticket, not an optional cleanup: Alex explicitly conditioned this resolution on CI provisioning staying additive (one name added to one array) rather than becoming a copy-pasted block per new database — see `G-008`'s Resolution section, "Reasoning" paragraph. A correct implementation makes `TEST_DB_NAMES` gaining a fifth or sixth entry someday require zero new lines in either workflow file. `.claude/hooks/session-start.sh` already loops this way (T-043) — mirror its shape rather than inventing a new one, adjusting only for GitHub Actions' `env:`-block limitation already noted in that script's own history (`Docs/IMPLEMENTATION_NOTES.md` § T-043 — a workflow's `env:` block can't reference a shell variable sourced inside `run:`, so per-database `DATABASE_URL` values get built inside the `run:` script, not `env:`).
...
Exit condition (machine-checkable):
  ...
  - adding a hypothetical fifth entry to `scripts/test-db-names.sh`'s `TEST_DB_NAMES` array and re-running the CI provisioning step locally (or via `act`/manual trace of the workflow YAML) creates and migrates that database without any further edit to `ci.yml` or `e2e-release-check.yml` — demonstrating the loop is genuinely generic, not a fixed-count unroll
