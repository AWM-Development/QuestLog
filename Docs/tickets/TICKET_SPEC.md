...
Runner: claude-code | devin   # which agent executes the ticket; see field notes

Model: sonnet            # only applies when Runner: claude-code; never opus/fable for execution
...
- **Runner** names which agent executes the ticket: `claude-code` or `devin`. Every ticket drafted before a second runner exists gets `claude-code` — `ticket-writer` defaults to it the same way it defaults `Priority` to `P1`, proposing rather than silently assuming. `Model:` only applies when `Runner: claude-code`; a `Runner: devin` ticket omits `Model:` or marks it N/A, since model selection there is Cognition's concern, not this pipeline's. This field is documented-but-inert until a second runner is actually wired — see `EXECUTOR_ROUTINE.md`'s "Runners" section for what does and doesn't read it yet.
- **Model: sonnet** is fixed, and only applies when `Runner: claude-code`. Planning and ticket-writing happen on Fable/Opus; execution never does.
