...
**(a) Worktree isolation.** Each executor session works in its own git worktree at `tmp/worktrees/T-###/`, never in the shared primary working directory. **Do not use `.claude/worktrees/` despite the naming precedent of the pre-existing `.claude/worktrees/heuristic-hermann-e69c56` directory** (created by unrelated harness tooling, not by this routine): `T-062` (`Docs/tickets/done/T-062-relocate-executor-markers-outside-claude-dir.md`) already hit this exact wall — the harness gates *any agent-initiated write* under `.claude/` behind an interactive confirmation prompt, which silently stalls an unattended nightly run waiting for approval that never comes. That's precisely why `tmp/.session-context.json`/`tmp/.active-ticket` live under `tmp/` today instead of `.claude/`, where T-061 originally put them. Creating a worktree is a write under that path the same way a file write is — `tmp/worktrees/T-###/` avoids the gate for the identical, already-proven reason. `tmp/` is not wiped between sessions (only two specific files under it are gitignored, not the directory itself), so a resumed ticket's worktree persists as expected.

Today `EXECUTOR_ROUTINE.md` Step 0 and `.claude/commands/executor.md:5` both run an unconditional `git checkout -B develop origin/develop`; the routine justifies this as safe because the sandbox is "a fresh, disposable workspace." True for a remote harness session, false for `/executor` run locally, where it force-moves `develop` and yanks the working tree out from under any concurrent session. Rewrite Step 0 and both command files accordingly.

**Before building the hand-rolled `git worktree add` flow below, check whether the harness's own subagent isolation does this job already.** The `Agent` tool exposes `isolation: "worktree"` — "creates a temporary git worktree so the agent works on an isolated copy of the repo," with automatic cleanup — which is almost certainly what actually produced the pre-existing `.claude/worktrees/heuristic-hermann-e69c56` directory. If a top-level `/executor` session can delegate its Step 2–7 implementation work to a subagent invoked with that isolation mode, it may be a more robust, already-battle-tested mechanism than reimplementing worktree lifecycle by hand — and it might sidestep the `.claude/` gate entirely, since it's the harness's own infrastructure creating the directory, not an agent-authored write, the same distinction that makes `tmp/worktrees/` safe versus `.claude/worktrees/` unsafe for hand-rolled writes. This may not be applicable — a top-level slash-command session and an `Agent`-tool-spawned subagent may not be interchangeable here — but confirm one way or the other before committing to the hand-rolled path, rather than assuming it's unavailable.
