...
| `/lineup` | none | Morning report: the next 3 eligible tickets (At Bat / On Deck / In the Hole), every open PR against `develop` awaiting Alex's review, and a full backlog snapshot with priority + blockers. Read-only. | No | Yes — safe to schedule daily |
