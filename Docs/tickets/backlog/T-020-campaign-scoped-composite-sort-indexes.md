...
plain `campaign_id` index this ticket's EXPLAIN baseline depends on, and
both tickets touch `apps/server/src/db/schema/tables.ts` / generate
migrations, so serializing avoids two in-flight schema diffs (same reasoning
as `T-015`/`T-016`).
...
Context files (load ONLY these):
  ...
  - Docs/tickets/done/T-012-entity-trgm-index-pre-filter.md — the
