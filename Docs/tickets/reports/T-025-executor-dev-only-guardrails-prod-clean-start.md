...
- ~~Item 2 of scope (verify prod starts clean) is not done and cannot be done from this session~~ — **addendum 2026-07-22: done.** Alex completed `Docs/DEPLOY_SETUP_CHECKLIST.md` §1–2 (Neon connection strings, Fly secrets, first deploys of both `questlog-dev` and `questlog-prod`, both from `main` after the `develop` → `main` merge that also carried PR #73's `release_command` path fix). With a real prod database live, this session ran the check the original item asked for: `campaign.create` → `campaign.list` against `https://questlog-prod.fly.dev` showed the created row as the *only* row present, then it was deleted directly via `mcp__Neon__run_sql` (Alex's explicit go-ahead — dev-only-guardrail item 1 has no bearing here since this was a human-directed interactive check, not the automated executor). Prod started genuinely clean, not just "no error was thrown."
- Per the ticket's own Definition of Done, `MILESTONES_V1_MCP.md`'s M-MCP.5 checkbox is now flipped in the same change that closes this — see that file.
