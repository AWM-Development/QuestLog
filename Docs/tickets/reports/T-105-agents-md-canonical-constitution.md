...
- **`AGENTS.md` exists at the repo root and contains every section `CLAUDE.md` carried before this ticket** — verified via `packages/core/src/ci/constitution-doc.ts`'s `checkConstitutionDoc()` run against the real repo files (`{ ok: true, failures: [] }`), plus a diff read confirming the content is byte-for-byte the prior `CLAUDE.md` (title line changed from `# CLAUDE.md` to `# AGENTS.md`, nothing else).
- **`CLAUDE.md` is ≤10 lines and contains the literal string `AGENTS.md`** — it's 6 lines; `checkConstitutionDoc()` confirms both facts.
...
2. **`packages/core/src/ci/constitution-doc.ts` is unwired** (reviewer's main finding) — it's a real, tested assertion of the exit condition's two hard facts, but has no `realDeps()`/CLI entry/`package.json` script/CI workflow step, unlike `gate-guard.ts`'s full pattern. Two options: wire it into CI for real as a small follow-up (same shape as `gate-guard.ts`'s `ci-gate-guard` wiring — plausibly in scope for `T-111`/`T-112`'s CI-guard series), or drop it in favor of the plain `grep`/`wc -l` the exit condition already specifies and treat this report's live-run evidence as sufficient. Left in place (green, harmless) pending that call.
