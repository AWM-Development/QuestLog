...
`apps/server/tsconfig.json` also currently has `outDir: "./dist"`, the same directory `apps/server/scripts/build.mjs`'s esbuild step writes the real deployable bundle to — the identical clobber bug T-028 already found and fixed for `apps/mcp-stdio` (via a separate `.typecheck-out` directory) is live and unfixed in `apps/server` today. Dropping composite/emit entirely removes this for free; keeping composite would need the same `.typecheck-out` treatment applied to `apps/server` as a separate, smaller fix regardless of how this gate resolves.
