...
- [ ] **M-EFFICIENCY.6 — PR-merge GitHub Action as ticket-status source of truth** (T-103)
  ...
  Exit: see T-103 — new `.github/workflows/ticket-status-ledger.yml` (with a `workflow_dispatch`/`dry_run` path for safe validation against a real merged PR, idempotent upsert verified); `EXECUTOR_ROUTINE.md` Step 1 and `TICKET_SPEC.md`'s "Why `develop`'s ticket directories can lag reality" section both updated to describe the ledger-first mechanism.

- [ ] **M-EFFICIENCY.7 — Audit GitHub Actions workflows for lean-ness ahead of M-1.1 gates** (T-104)
