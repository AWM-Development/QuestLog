...
`G-006` (`Docs/tickets/gated/resolved/G-006-entity-delete-archive-semantics.md`) — whether removing an entity should be a soft-archive or a hard delete, and how references from `session_entities`/`entity_relationships` should be handled — was resolved via `/ungate` on 2026-07-30 (soft-archive, new `entities.status` column mirroring `campaigns`; no cascade/block logic needed since the row never disappears; writes against an archived entity remain allowed; an unarchive path is required). M-REMOTE.10's tag below is cleared accordingly: schema + service work is T-088 (`queue/`), the MCP tool pair is T-089 (`backlog/`, blocked on T-088's merge).
...
- [ ] **M-REMOTE.10 — Entity delete/archive MCP tool** (T-088, T-089)
  Raised during T-032's morning review, alongside M-REMOTE.9. Resolved via `/ungate` (G-006, 2026-07-30): soft-archive, new `entities.status` column mirroring `campaigns`, no cascade/block logic needed since `session_entities`/`entity_relationships` rows keep resolving against an archived entity as before. Split into two tickets: T-088 (schema + service: `archive`/`unarchive`, `list` excludes archived by default) and T-089 (MCP `archive_entity`/`unarchive_entity` preview/confirm tool pairs, blocked on T-088).
  Exit: both T-088 and T-089 ship.
