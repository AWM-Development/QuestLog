...
**Open gates:** G-045 (`delete_source` tool design), G-046 (`ingest_text` idempotency-key strategy), G-053 (composable sub-entities for content-heavy parent entities).
...
- [ ] **M-BUG.8 — Entity `description` has no supported reorder/mid-document-insert path once content-heavy** (Gated on: G-053)
