...
- [x] **M-MCP.5 — Deploy v1 (MCP server + backend + DB) to dev and production** ✅ **done 2026-07-22.** `questlog-dev` and `questlog-prod` (Fly.io) both deployed and verified against their own Neon branch: schema migrated, pgvector/pg_trgm extensions live, a real write/read/delete round-trip proven through `campaign.create`/`campaign.list`, and prod confirmed to start genuinely clean (no pre-existing rows). One real bug found and fixed en route: `release_command` in both `fly.*.toml` used a repo-root-relative path that doubled against the Dockerfile's `WORKDIR /repo/apps/server`, failing the first dev deploy (PR #73). Sub-tickets: T-023 (readiness audit), T-024 (environment/DB setup), T-025 (dev-only guardrails + prod clean-start check — see that ticket's report addendum for the verification detail). **v1 is fully shipped** — all of M-MCP.0–5 done, `apps/mcp` is reachable against both a local dev checkout and now real hosted infrastructure.
