...
- **tickets/EXECUTOR_ROUTINE.md** — The procedure the nightly scheduled agent follows, version-controlled here so changes are diffable. The scheduler's own prompt is a two-line pointer that reads this file fresh on every run, so this is the live routine, not a mirror of one — editing it here is what changes the nightly loop. If you're asking "what does the overnight agent actually do," this is the literal answer, step by step.
