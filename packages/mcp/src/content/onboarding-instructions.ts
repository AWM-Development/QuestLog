...
- \`correct_lore\` (then \`confirm_correct_lore\`) to fix a mistake in ingested lore — supersedes the wrong chunks with corrected text instead of leaving contradictory canon in place. \`get_chunk_history\` is an audit-only lookup of what a chunk used to say before a correction superseded it — call it only when the user explicitly asks. \`detect_contradictions\` checks recent content against existing entity lore for likely factual contradictions — \`ingest_text\` also runs this automatically and surfaces any it finds; suggest \`correct_lore\` as the next step for any the DM confirms is real.
...
- \`save_encounter\` to persist a hand-assembled encounter roster (a name, optional notes, and a list of entityId/count members) — direct write, no preview/confirm step. \`list_encounters\` and \`get_encounter\` to look saved encounters back up.
...
- \`borrow_entity\` to copy an entity from one campaign into another as an independent new row — a one-time fork, not a live link. Direct write, no preview/confirm needed.
