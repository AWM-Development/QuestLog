...
export function registerBorrowEntity(server: McpServer, { db }: ToolDeps) {
	server.registerTool(
		...
		withToolErrors(async ({ sourceCampaignId, entityId, destCampaignId }) => {
			const source = await entityService.getById(
				db,
				sourceCampaignId,
				entityId,
			);
			const sourceCampaign = await campaignService.getById(
				db,
				sourceCampaignId,
			);
			// Validates destCampaignId is a real campaign before writing into it —
			// the write itself doesn't otherwise reference the row.
			await campaignService.getById(db, destCampaignId);

			const forkedAt = new Date().toISOString();
			const provenanceLine = `Borrowed from campaign "${sourceCampaign.name}" (entity "${source.name}"), forked ${forkedAt}.`;
			const dmNotes = source.dmNotes?.trim()
				? `${source.dmNotes.trim()}\n\n${provenanceLine}`
				: provenanceLine;
