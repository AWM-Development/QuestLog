...
- **New `confirm_correct_lore` MCP tool (apply half).** Confirms a `correct_lore` preview token: in one transaction, chunks and embeds the correction as new authoritative content and marks every target chunk `superseded`. Returns the created and superseded chunk ids. A second confirm against the same token is rejected (same claim semantics as `confirm_log_session`). Completes the M-CANON preview→confirm pair with T-075.
