...
- **Observability dashboard: Log view + comments.** New `/log` route on `apps/observability-dashboard` (T-057's app shell): a reverse-chronological ticket-run feed fetched from `observability.feed` (T-054) joined client-side against `observability.trends` for each ticket's cost/tokens/tier, since neither read endpoint's shape changed for this ticket. Each entry shows outcome and reviewer-verdict badges, a one-line summary, an always-visible "Efficiency notes" aside, and cost/tokens, and expands (`<details>`, matching the mockup) into the full report — test evidence, exit-condition checks, the reviewer's verbatim verdict, and efficiency notes (including the retry log), all extracted from the report's markdown via a new `features/log/utils/parseReport.ts`. A blocked entry renders with distinct styling and swaps in the blocked-report shape (what was attempted, why it stopped) plus an "Exact question for Alex" callout, open by default. A functional outcome filter (all/shipped/blocked) narrows the rendered list against the fetched data. Every entry carries a comment thread (T-059's `comment.list`/`comment.add` endpoints) — existing comments plus an add-comment form that appends the new comment on success without a full page reload. Alex-authored comments only for v1; agent-authored comments remain deferred. Completes `M-OBS.5`.
...
- **`session-start.sh` split into three separate `SessionStart` hooks, by responsibility.** The single ~370-line script conflated git/session hygiene, local worktree Postgres provisioning, and remote-sandbox Postgres provisioning — three unrelated jobs that happened to all need to run at session start. Now `.claude/hooks/session-git-hygiene.sh`, `session-db-local.sh`, and `session-db-remote.sh`, registered as three entries under `.claude/settings.json`'s `SessionStart` array (Claude Code runs multiple hook entries for the same event in parallel — the documented shape, not a workaround). No behavior change: each DB script still no-ops immediately outside its own case (worktree-vs-primary, local-vs-remote), and the fast-path/verification-gate logic is unchanged. Full rationale: `IMPLEMENTATION_NOTES.md` § "session-start.sh split into three SessionStart hooks".
