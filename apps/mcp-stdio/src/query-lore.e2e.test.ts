 ...
 * apps/server/src/search.e2e.test.ts: uploads the permanent
...
// apps/mcp-stdio is a leaf (nothing depends on it), so unlike packages/core
// and packages/mcp it can safely still reach into apps/server directly for
// buildApp — the ad hoc "@questlog/server/*" wildcard mapping this used to
// need for db/mcp/services too, now scoped to just this one remaining case.
